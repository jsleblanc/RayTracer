...
module SphereTests =
    ...
    let ``The normal on a sphere at a nonaxial point``() =
        ...
        let s = RenderLib.ShapeSphere.build
        let n = s.local_normal_at s (point v v v)
    ...
    let ``Computing the normal on a translated sphere``() =
        //TODO - should be normal_at function, not local_normal_at
        let s = RenderLib.ShapeSphere.build
        let s = transform s (translation 0.0 1.0 0.0)
        let n = s.local_normal_at s (point 0.0 1.70711 -0.70711)
    ...
    let ``Computing the normal on a transformed sphere``() =
        //TODO - should be normal_at function, not local_normal_at
        let s = RenderLib.ShapeSphere.build
        let s = transform s ((scaling 1.0 0.5 1.0) * rotation_z (Math.PI / 5.0))
        let n = s.local_normal_at s (point 0.0 (Math.Sqrt(2.0)/2.0) (-Math.Sqrt(2.0)/2.0))
