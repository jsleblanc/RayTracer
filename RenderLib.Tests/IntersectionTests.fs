...
module IntersectionTests = 
    ...
    let ``An intersection encapsulates t and object``() =
        ...
        let i = Shapes2.build_intersection 3.5 s []
        Assert.Equal(s, i.obj)

    [<Fact>]
    let ``The hit, when all intersections have positive t``() =
        let s = ShapeSphere.build
        let i1 = Shapes2.build_intersection 1.0 s []
        let i2 = Shapes2.build_intersection 2.0 s []
        let xs = [ i1; i2; ]
        match hit xs with
    ...
    let ``The hit, when some intersections have negative t``() =
        let s = ShapeSphere.build
        let i1 = Shapes2.build_intersection -1.0 s []
        let i2 = Shapes2.build_intersection 1.0 s []
        let xs = [ i1; i2; ]
        match hit xs with
    ...
    let ``The hit is always the lowest non-negative intersection``() =
        let s = ShapeSphere.build
        let i1 = Shapes2.build_intersection 5.0 s []
        let i2 = Shapes2.build_intersection 7.0 s []
        let i3 = Shapes2.build_intersection -3.0 s []
        let i4 = Shapes2.build_intersection 2.0 s []
        let xs = [ i1; i2; i3; i4; ]
        match hit xs with
    ...
    let ``The hit, when an intersection occurs on the inside``() =
        ...
        let s = ShapeSphere.build
        let i = Shapes2.build_intersection 1.0 s []
        let comps = prepare i r [i]
        Assert.Equal(point 0.0 0.0 1.0, comps.point)
        Assert.Equal(vector 0.0 0.0 -1.0, comps.eyev)
        Assert.Equal(vector 0.0 0.0 -1.0, comps.normalv)
    ...
    let ``The hit should offset the point``() =
        ...
        let s = ShapeSphere.build |> Shapes2.transform (translation 0.0 0.0 1.0)
        let i = Shapes2.build_intersection 5.0 s []
        let comps = prepare i r [i]
    ...
    let ``Finding n1 and n2 at various intersections``(index, n1, n2) =
        let a = ShapeSphere.build |> Shapes2.texture glass |> Shapes2.transform (scaling 2.0 2.0 2.0)
        let b = ShapeSphere.build |> Shapes2.texture { glass with refractive_index = 2.0; } |> Shapes2.transform (translation 0.0 0.0 -0.25)
        let c = ShapeSphere.build |> Shapes2.texture { glass with refractive_index = 2.5; } |> Shapes2.transform (translation 0.0 0.0 0.25)
        let r = {
            origin = point 0.0 0.0 -4.0;
            direction = vector 0.0 0.0 1.0;
        }
        let xs = [
            Shapes2.build_intersection 2.0 a [];
            Shapes2.build_intersection 2.75 b [];
            Shapes2.build_intersection 3.25 c [];
            Shapes2.build_intersection 4.75 b [];
            Shapes2.build_intersection 5.25 c [];
            Shapes2.build_intersection 6.0 a [];
        ]
    ...
    let ``The under point is offset below the surface``() =
        ...
        let s = ShapeSphere.build |> Shapes2.texture glass |> Shapes2.transform (translation 0.0 0.0 1.0)
        let i = Shapes2.build_intersection 5.0 s []
        let xs = [i]
    ...
    let ``The Schlick approximation under total internal reflection``() =
        let s = ShapeSphere.build |> Shapes2.texture glass
        let r = {
            origin = point 0.0 0.0 (Math.Sqrt(2.0)/2.0);
            direction = vector 0.0 1.0 0.0;
        }
        let xs = [
            Shapes2.build_intersection (-Math.Sqrt(2.0)/2.0) s [];
            Shapes2.build_intersection (Math.Sqrt(2.0)/2.0) s [];
        ]
    ...
    let ``The Schlick approximation with a perpendicular viewing angle``() =
        ...
        let xs = [
            Shapes2.build_intersection -1.0 s [];
            Shapes2.build_intersection 1.0 s [];
        ]
        let comps = prepare (Seq.item(1) xs) r xs
        let reflectance = schlick comps
    ...
    let ``The Schlick approximation with small angle and n2 > n2``() =
        ...
        let xs = [
            Shapes2.build_intersection 1.8589 s [];
        ]
